...
async def balance(context: commands.Context):
    ...
    await bot.send_message(
        ...
        f'💰 Available: {wallet.actual_balance / COIN_DIGITS:.7f} '
        f'{COIN_REPR}\n'
        f'👛 Pending: {wallet.locked_balance / COIN_DIGITS:.7f} '
        f'{COIN_REPR}\n')
...
async def botbalance(context: commands.Context, member: discord.Member):
    if (member.bot == False):
        ...
        wallet = store.get_user_wallet(user.user_id)
        balance_actual = '{:,.7f}'.format(wallet.actual_balance / COIN_DIGITS)
        balance_locked = '{:,.7f}'.format(wallet.locked_balance / COIN_DIGITS)
        await bot.send_message(
...
async def register(context: commands.Context, wallet_address: str):
    ...
    if not re.match(r'cat1[a-zA-Z0-9]{94,}', wallet_address.strip()):
        await bot.add_reaction(context.message, EMOJI_ERROR)
        await bot.send_message(context.message.author,
                        f'🛑 Invalid address:\n'
                        f'`{wallet_address}`')
    ...
    if real_amount > config.max_tx_amount:
        ...
        await bot.send_message(
            context.message.author,
                ...
                f'{config.max_tx_amount / COIN_DIGITS:.7f} '
                f'{COIN_REPR}')
        return
        ...
        await bot.send_message(
            context.message.author,
                ...
                f'{config.min_tx_amount / COIN_DIGITS:.7f} '
                f'{COIN_REPR}')
        return
...
async def _tip(context: commands.Context, amount,
    ...
    await bot.send_message(
        context.message.author,
                    ...
                    f'Transaction hash: `{tip.tx_hash}`\n'
                    f'Each: `{real_amount / COIN_DIGITS:.7f}{COIN_REPR}`'
                    f'Total spending: `{ActualSpend / COIN_DIGITS:.7f}{COIN_REPR}`')
    for member in context.message.mentions:
        ...
        if (context.message.author.id != member.id) :
            if (member.bot == False):
                await bot.send_message(
                    member,
                        f'💰 You got a tip of {real_amount / COIN_DIGITS:.7f} '
                        f'{COIN_REPR} from `{context.message.author.name}`\n'
                        f'Transaction hash: `{tip.tx_hash}`')
    return
